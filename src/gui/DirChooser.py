...
from gui.CustomWidgets import TimerProgressBar
...
class DirChooser(Step):
	...
	def __init__(self, height=150, intro_wait=3.5, title='Import files from: '):
		super().__init__()
		self.title = title
		self.height = height
	...
	def _update(self, box):
		...
		box.children = tuple([widgets.HTML("<h4>" + self.title + "</h4>")] +
							 [widgets.HTML("<p><b>Current directory: </b>%s</p>" % (self.path,))] + [carousel] + [file_filter] + [confirm])
