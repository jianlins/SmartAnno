...
from ehost.EhostCorpusReader import EhostCorpusReader
...
class PreProcessing:
	...
	def vectorize(self, sentences, labels=[]):
		...
		x = np.array([[self.glove_model.word_vec(word) if word in self.glove_model.vocab
					   else np.random.uniform(-0.25, 0.25, self.glove_model.vector_size) for word in sentence] for sentence in sentences])
		y = None
		if len(labels) > 0:
